...
public class LiquidFuzeItem extends AbstractPathScanFuzeItem {
    public LiquidFuzeItem(Properties properties) {
        super(properties);
    }
    @Override
    public boolean detonateCheck(Level level, Vec3 p, ItemStack stack, AbstractCannonProjectile projectile) {
        return !level.getFluidState(new BlockPos((int) p.x, (int) p.y, (int) p.z)).isEmpty();
    }
    @Override
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.translatable("tooltip.cbc_phantom_shells.liquid_fuze.0"));
    }
}
