...
public class AirFuzeItem extends AbstractPathScanFuzeItem {
    public AirFuzeItem(Properties properties) {
        super(properties);
    }
    @Override
    public boolean detonateCheck(Level level, Vec3 p, ItemStack stack, AbstractCannonProjectile projectile) {
        return level.getBlockState(new BlockPos((int) p.x, (int) p.y, (int) p.z)).isAir();
    }
    @Override
    public void appendHoverText(ItemStack stack, @Nullable Level level, List<Component> tips, TooltipFlag flag) {
        ...
        tips.add(Component.translatable("tooltip.cbc_phantom_shells.air_fuze.0"));
    }
}
